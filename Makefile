...
LDFLAGS_BUILD = -ldflags "-X $(SERVICE_PATH).BuildTime=$(BUILD_TIME) -X $(SERVICE_PATH).GitCommit=$(GIT_COMMIT) -X $(SERVICE_PATH).Version=$(VERSION) -X github.com/ONSdigital/dp-frontend-release-calendar/config.RendererVersion=$(APP_RENDERER_VERSION)"
...
build: generate-prod ## Builds binary of application code and stores in bin directory as dp-frontend-release-calendar
	go build -tags 'production' $(LDFLAGS_BUILD) -o $(BINPATH)/dp-frontend-release-calendar
...
debug: generate-debug ## Used to build and run code locally in debug mode
	go build -tags 'debug' $(LDFLAGS_DEBUG) -o $(BINPATH)/dp-frontend-release-calendar
